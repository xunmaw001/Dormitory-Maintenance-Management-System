...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"2"}'>
      ...
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>维修进度</div>
        <el-select v-model="formSearch.weixiujindu" placeholder="请选择维修进度" :clearable="true">
          <el-option v-for="(item, index) in weixiujinduOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.weixiujindu != '') searchWhere.weixiujindu = this.formSearch.weixiujindu;
        this.$http.get('weixiujindu/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/weixiujinduDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
